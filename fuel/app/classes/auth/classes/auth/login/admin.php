...
/**
 * admin basic login driver
 *
 * @package     Fuel
 * @subpackage  Auth
 */
class Auth_Login_Admin extends \Auth_Login_Driver
{
	/**
	...
	public static function _init()
	{
		...
		if (\Config::get('admin.remember_me.enabled', false))
		{
			static::$remember_me = \Session::forge(array(
				'driver' => 'cookie',
				'cookie' => array(
					'cookie_name' => \Config::get('admin.remember_me.cookie_name', 'rmcookie'),
				),
				'encrypt_cookie' => true,
				'expire_on_close' => false,
				'expiration_time' => \Config::get('admin.remember_me.expiration', 86400 * 31),
			));
		}
	}
	...
	protected function perform_check()
	{
		...
		if ( ! empty($username) and ! empty($login_hash))
		{
			...
			{
				$this->user = \DB::select_array(\Config::get('admin.table_columns', array('*')))
					->where('username', '=', $username)
					->from(\Config::get('admin.table_name'))
					->execute(\Config::get('admin.db_connection'))->current();
			}

			// return true when login was verified, and either the hash matches or multiple logins are allowed
			if ($this->user and (\Config::get('admin.multiple_logins', false) or $this->user['login_hash'] === $login_hash))
			{
				return true;
			}
		}
		...
		$this->user = \Config::get('admin.guest_login', true) ? static::$guest_login : false;
	...
	{
		$username_or_email = trim($username_or_email) ?: trim(\Input::post(\Config::get('admin.username_post_key', 'username')));
		$password = trim($password) ?: trim(\Input::post(\Config::get('admin.password_post_key', 'password')));
		...
		$user = \DB::select_array(\Config::get('admin.table_columns', array('*')))
			->where_open()
	...
	public function force_login($user_id = '')
	{
		...
		$this->user = \DB::select_array(\Config::get('admin.table_columns', array('*')))
			...
			->where_close()
			->from(\Config::get('admin.table_name'))
			->execute(\Config::get('admin.db_connection'))
			->current();
	...
	public function create_user($username, $password, $email, $group = 1, Array $profile_fields = array())
	{
		...
		$same_users = \DB::select_array(\Config::get('admin.table_columns', array('*')))
			->where('username', '=', $username)
...
//			'group'           => (int) $group,
		...
		$result = \DB::insert(\Config::get('admin.table_name'))
			->set($user)
			->execute(\Config::get('admin.db_connection'));
	...
	public function update_user($values, $username = null)
	{
		...
		$current_values = \DB::select_array(\Config::get('admin.table_columns', array('*')))
			->where('username', '=', $username)
			->from(\Config::get('admin.table_name'))
			->execute(\Config::get('admin.db_connection'));
		...
		$affected_rows = \DB::update(\Config::get('admin.table_name'))
			->set($update)
			->where('username', '=', $username)
			->execute(\Config::get('admin.db_connection'));
	...
	public function delete_user($username)
	{
		...
		$affected_rows = \DB::delete(\Config::get('admin.table_name'))
			->where('username', '=', $username)
			->execute(\Config::get('admin.db_connection'));
	...
	public function create_login_hash()
	{
		...
		$login_hash = sha1(\Config::get('admin.login_hash_salt').$this->user['username'].$last_login);

		\DB::update(\Config::get('admin.table_name'))
	...
	{
		return \Config::get('admin.guest_login', true);
	}
}
